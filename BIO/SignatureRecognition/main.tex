...
As we can observe, we could say that the distributions of \(Ttotal, Tpendown\) and \(Ppendown\) are Gaussian distributions for certain parameters \(\mu,\sigma\). The variable \(Npenups\) is a little bit different, we could say that it is a \emph{bimodal} distribution, such as a Gaussian Mixture of \(2\) gaussians.
