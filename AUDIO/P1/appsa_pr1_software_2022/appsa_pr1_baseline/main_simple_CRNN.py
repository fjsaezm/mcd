...
if __name__ == '__main__':
    ...
    if not no_weak:
        weak_df = dataset.initialize_and_get_df(cfg.weak, reduced_number_of_data, download=False)

    synthetic_df = dataset.initialize_and_get_df(cfg.synthetic, reduced_number_of_data, download=False)
    validation_df = dataset.initialize_and_get_df(cfg.validation, reduced_number_of_data, download=False)
    ...
    if not no_weak:
        ...
        classes = DatasetDcase2019Task4.get_classes([validation_df, synthetic_df])
    ...
    class_names = ['Alarm_bell_ringing', 'Blender', 'Cat', 'Dishes', 'Dog', 
                  'Electric_shaver_toothbrush', 'Frying', 'Running_water', 'Speech', 'Vacuum_cleaner']
    results = pd.DataFrame(columns=["valid_synth_f1", "weak_metric", "global_valid"])
    results_perclass = pd.DataFrame(columns= ['Total_macro', 'Total_micro'] + class_names)
    ...
    for epoch in range(cfg.n_epoch):
        ...
        valid_synth_f1 = valid_metric.results_class_wise_average_metrics()['f_measure']['f_measure']
        ...
        for c in class_names:
            class_f = valid_metric.class_wise_f_measure(c)['f_measure']
            results_perclass.loc[epoch, c] = class_f
        results_perclass.loc[epoch, 'Total_micro'] = valid_metric.overall_f_measure()['f_measure']
        results_perclass.loc[epoch, 'Total_macro'] = valid_synth_f1
    ...
    results_perclass_df = pd.DataFrame(results_perclass).to_csv(os.path.join(saved_pred_dir, "results_perclass.tsv"),
